 ...
 * 
 */
public class DataAdapter3 extends BaseDataAdapter {
	
	public DataAdapter3(Context context, DataSource dataSource, int resource,
			String[] from, int[] to) {
	...
	protected void bindView(int position, View view) {
		...
		for (int i = 0; i < count; i++) {
			...
			if (v != null) {
				...
				if (i == 2) {// NOTE:change the string which present the state
								// of exams and the index of zIds is 1
					if (text.equals(String.valueOf(choice.answerState_waitMark))) {
						text = StateStr.CORRECT;
					} else if (text.equals(String.valueOf(State.CORRECTED))) {
						text = StateStr.CORRECTED;
					} else if (text.equals(String
							.valueOf(choice.answerState_waitAnser))) {
						text = StateStr.HANDIN;
					} else if (text.equals(String.valueOf(State.HANDINED))) {
						text = StateStr.HANDINED;
					}
				}
